...
func main() {
	...
	r.HandleFunc("/api/shorten", logRequest(handlers.CreateShortURLHandler(db, urlCache))).Methods("POST")
	r.HandleFunc("/{shortCode}", logRequest(handlers.RedirectHandler(db, urlCache))).Methods("GET")
	r.HandleFunc("/", logRequest(handlers.WebInterfaceHandler(db))).Methods("GET", "POST")

	//// Проверка кеша
	//fmt.Println("Получение записей из кеша...")
	...
	//}
	//
	//fmt.Println("Удаление записи short2 из кеша...")
	...
	r.HandleFunc("/register", logRequest(auth.SignUp(db))).Methods("POST")
	r.HandleFunc("/login", logRequest(auth.AuthMiddleware((auth.Login(db))))).Methods("POST")
